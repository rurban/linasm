...
;---[Initialization of hash array]---------
	...
	punpcklqdq	vsize, vsize				; duplicating size through the entire register
		mov		count, hash_sz				; count = hash_sz
;---[Initialization loop]------------------
.iloop:	movdqa	[hptr + 0x00], vsize		; hptr[0] = vsize
		movdqa	[hptr + 0x10], vsize		; hptr[16] = vsize
		movdqa	[hptr + 0x20], vsize		; hptr[32] = vsize
		movdqa	[hptr + 0x30], vsize		; hptr[48] = vsize
...
;------------------------------------------
		...
		test	psize, bytes - 1			; if elements have wrong alignment
...
if scale > 0
		test	p_src, bytes - 1			; if element has wrong alignment
